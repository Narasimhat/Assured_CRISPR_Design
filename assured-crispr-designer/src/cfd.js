// Cutting Frequency Determination (CFD), Doench et al. 2016, Nat Biotechnol 34:184-191,
// "Optimized sgRNA design to maximize activity and minimize off-target effects of
// CRISPR-Cas9". Supplementary Tables 19/20.
//
// CFD estimates how much cleavage activity a mismatched target retains, relative to a
// perfect match with an NGG PAM. It is the published, position- and identity-weighted answer
// to the question this engine previously answered by counting mismatches: a mismatch at the
// PAM-proximal end and one at the far end were graded identically, which the underlying
// data does not support.
//
// Applied here to the *repaired allele*: the original guide scored against the donor's
// edited target site. That is the same physical situation CFD was fitted on - one guide,
// one imperfectly matched DNA target - so it transfers directly.
//
// Limits worth keeping in view: CFD is SpCas9-only, was fitted on off-target cleavage in a
// single screen (CD33 in MOLM13), and is a relative activity estimate rather than a
// probability. It says nothing about repair outcome.
//
// This file is GENERATED from the tables distributed with the CFD calculator, decoded and
// checked (240 mismatch entries, 16 PAM entries, positions 1-20, all values in [0,1], PAM
// values cross-checked against independently published figures). Do not hand-edit it.

/** Relative activity by mismatch identity and position. Key: "r<guide>:d<target>,<pos>". */
export const CFD_MISMATCH = Object.freeze({
  "rA:dA,1": 1.0,
  "rA:dA,2": 0.727272727,
  "rA:dA,3": 0.705882353,
  "rA:dA,4": 0.636363636,
...
/** Relative activity by PAM positions 2-3. NGG = 1 by definition. */
export const CFD_PAM = Object.freeze({
  AA: 0.0,
  AC: 0.0,
...
const RNA_OF = { A: "A", C: "C", G: "G", T: "U", U: "U" };
const COMPLEMENT = { A: "T", C: "G", G: "C", T: "A", U: "A" };

/**
 * CFD for one guide against one target site.
 *
 * @param {string} guideSpacer  the 20 nt spacer as designed
 * @param {string} targetSpacer the 20 nt target, same orientation as the spacer
 * @param {string} pam          the target PAM; positions 2-3 are used
 * @returns {{score: number, mismatches: number[], pamKey: string}|null} null when the
 *   inputs are not scoreable, so a caller cannot mistake a failure for a low score.
 */
export function calculateCfd(guideSpacer, targetSpacer, pam) {
  const guide = String(guideSpacer || "").toUpperCase();
  const target = String(targetSpacer || "").toUpperCase();
  const pamKey = String(pam || "").toUpperCase().slice(-2);
  if (guide.length !== 20 || target.length !== 20) return null;
  if (!/^[ACGT]{20}$/.test(guide) || !/^[ACGT]{20}$/.test(target)) return null;
  if (!Object.prototype.hasOwnProperty.call(CFD_PAM, pamKey)) return null;

  let score = CFD_PAM[pamKey];
  const mismatches = [];
  for (let index = 0; index < 20; index += 1) {
    if (guide[index] === target[index]) continue;
    // The published key encodes the guide base as RNA and the target base complemented -
    // the convention of the reference implementation. A key that does not exist means the
    // lookup is wrong, so it fails loudly instead of quietly scoring 1.
    const key = `r${RNA_OF[guide[index]]}:d${COMPLEMENT[target[index]]},${index + 1}`;
    if (!Object.prototype.hasOwnProperty.call(CFD_MISMATCH, key)) return null;
    score *= CFD_MISMATCH[key];
    mismatches.push(index + 1);
  }
  return { score, mismatches, pamKey };
}
